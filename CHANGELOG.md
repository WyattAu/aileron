...
- **`src/servo/wry_engine.rs`**: gated the `AtomicBool`/`Ordering` import behind `#[cfg(target_os = "linux")]` (they back the Linux-only `WEBVIEW_ACCEPTS_FOCUS` X11-focus gate), and added a non-Linux no-op consumption of the `parent_x`/`parent_y` parameters in `set_bounds_with_parent_offset` (they are only read by the GTK offset block). Both previously failed clippy `-D warnings` on macOS/Windows.
- **`src/offscreen_webview.rs`** and **`src/servo/wry_pages.rs`**: annotated the Linux-only `bgra_buffer` field and `aileron_welcome_page` function with `#[cfg_attr(not(target_os = "linux"), allow(dead_code))]`; both are reached only via the Linux offscreen custom-protocol handler and were flagged as dead code on macOS/Windows.
- **`src/platform/macos.rs`**: prefixed the unused `title` parameter of `file_open_dialog` with `_` (the macOS AppleScript implementation does not use it).
- **`src/db/mod.rs`**: gated the `tracing::warn` import behind `#[cfg(unix)]`; `warn!` is only invoked inside the `#[cfg(unix)]` file-permissions block and was flagged as an unused import on Windows.
