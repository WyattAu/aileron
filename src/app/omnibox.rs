...
impl AppState {
    pub fn update_omnibox(&mut self, query: &str) {
        self.ui.omnibox_results.clear();
        self.ui.omnibox_selected = 0;

        let query = query.trim();
        if query.is_empty() {
            self.ui.last_omnibox_query.clear();
            return;
        }

        self.ui.last_omnibox_query = query.to_string();
        ...
        self.ui.omnibox_results = scored.into_iter().take(10).map(|s| s.item).collect();
    }

    pub fn handle_omnibox_select(&mut self, index: usize) {
        if let Some(item) = self.ui.omnibox_results.get(index) {
            let id = item.id.clone();
            let label = item.label.clone();
            if let Some(url_str) = id.strip_prefix("nav:") {
                if let Ok(url) = url::Url::parse(url_str) {
                    self.navigate_with_redirects(url);
                    self.ui.status_message = format!("Navigating to {url_str}");
                }
                ...
                if let Ok(parsed) = url::Url::parse(url) {
                    ...
                    self.ui.status_message = format!("Opening bookmark: {label}");
                }
            } else if let Some(url) = id.strip_prefix("history:")
                && let Ok(parsed) = url::Url::parse(url)
            {
                self.navigate_with_redirects(parsed);
                self.ui.status_message = format!("Opening: {url}");
                ...
                && let Ok(parsed) = url::Url::parse(url)
            {
                ...
                if !switched {
                    ...
                    self.ui.status_message = format!("Switched to tab: {label}");
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_empty_query_clears_results() {
        ...
        state
            .ui
            .omnibox_results
            .push(crate::ui::search::SearchItem {
                id: "test:1".into(),
                label: "Test".into(),
        ...
        assert!(state.ui.omnibox_results.is_empty());
        assert_eq!(state.ui.omnibox_selected, 0);
        assert!(state.ui.last_omnibox_query.is_empty());
    }
    ...
    fn test_url_detection() {
        ...
        assert_eq!(state.ui.omnibox_results.len(), 1);
        assert!(state.ui.omnibox_results[0].id.starts_with("nav:"));
        assert_eq!(state.ui.omnibox_results[0].label, "https://example.com");
    }
    ...
    fn test_non_url_triggers_search() {
        ...
        assert_eq!(state.ui.omnibox_results.len(), 1);
        assert!(state.ui.omnibox_results[0].id.starts_with("search:"));
        assert!(state.ui.omnibox_results[0].label.contains("hello world"));
    }
    ...
    fn test_selected_reset_on_new_query() {
        ...
        state.ui.omnibox_selected = 5; // Simulate user selecting item 5
    ...
    fn test_results_capped_at_10() {
        ...
        assert!(state.ui.omnibox_results.len() <= 11); // 1 nav + max 10 others
    }
}
